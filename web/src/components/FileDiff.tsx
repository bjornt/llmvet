...
export default function FileDiff({
    ...
    const widgets = useMemo(() => {
        ...
        for (const hunk of hunks) {
            for (const change of (hunk as HunkData).changes) {
                ...
                const isComposing =
                    composerState &&
                    composerState.file === file.path &&
                    composerState.changeKey === key;
                ...
                w[key] = (
                    <div className="max-w-[100ch] border-t border-slate-200 bg-white dark:border-slate-600 dark:bg-slate-900">
                        {lineComments?.map((c) => (
                            <div
                                key={c.localId}
                                className="border-b border-slate-100 px-4 py-2 last:border-b-0 dark:border-slate-700"
                            >
    ...
    const renderGutter: RenderGutter = useCallback(
        ({ change, side, inHoverState, renderDefault, wrapInAnchor }) => {
            ...
            const commentLine = hasCurrentLine
                ? currentLine
                : oldLine !== -1
                  ? oldLine
                  : newLine;
            const commentSide = hasCurrentLine
                ? (side as "old" | "new")
                : oldLine !== -1
                  ? "old"
                  : "new";
            ...
            return wrapInAnchor(
                <span
                    className={
                        "relative flex items-center px-2 w-full min-h-[1.25rem]" +
                        (hasLine ? " cursor-pointer" : "")
                    }
    ...
    useEffect(() => {
        ...
        const handler = (e: MouseEvent) => {
            const cell = (e.target as HTMLElement).closest(
                ".diff-gutter-omit, .diff-code-omit",
            );
            if (!cell) return;
            ...
            const side: "old" | "new" =
                cells.indexOf(keyCell) < 2 ? "old" : "new";
            const lineText = keyCell.textContent?.trim();
            const line = lineText ? parseInt(lineText, 10) : 0;

            onStartComposerRef.current(
                file.path,
                keyCell.dataset.changeKey,
                line,
                side,
            );
        };
