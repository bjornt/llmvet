import type { ChangeData, HunkData, DiffType, TokenNode } from 'react-diff-view';
...
export function detectLanguage(path: string): string | undefined {
  ...
  const ext = basename.includes('.') ? basename.split('.').pop()!.toLowerCase() : basename;
  const lang = EXT_LANG[ext] ?? EXT_LANG[basename];
  if (lang && typeof refractor.registered === 'function' && refractor.registered(lang)) {
    return lang;
  }
  if (lang) {
    try {
...
function generateHunkContent(hunk: { old_start: number; old_lines: number; new_start: number; new_lines: number }): string {
  const oldLen = hunk.old_lines;
  const newLen = hunk.new_lines;
  return `@@ -${hunk.old_start},${oldLen} +${hunk.new_start},${newLen} @@`;
}

function mapLineToChange(line: { type: string; old: number | null; new: number | null; content: string }): ChangeData {
...
export function mapHunk(hunk: { old_start: number; old_lines: number; new_start: number; new_lines: number; lines: { type: string; old: number | null; new: number | null; content: string }[] }): HunkData {
  return {
...
export function tokenizeHunks(hunks: HunkData[], language: string): { old: TokenNode[][]; new: TokenNode[][] } {
  const oldMax = Math.max(
    0,
    ...hunks.flatMap(h =>
      h.changes.map(c =>
        c.type === 'normal' ? c.oldLineNumber : c.type === 'delete' ? c.lineNumber : -1,
      ).filter(n => n > 0),
    ),
  );
  const newMax = Math.max(
    ...
    ...hunks.flatMap(h =>
      h.changes.map(c =>
        c.type === 'normal' ? c.newLineNumber : c.type === 'insert' ? c.lineNumber : -1,
      ).filter(n => n > 0),
    ),
  );
  ...
  for (const hunk of hunks) {
    for (const change of hunk.changes) {
      ...
      if (change.type === 'normal') {
        if (change.oldLineNumber! > 0) oldLines[change.oldLineNumber! - 1] = tokens;
        if (change.newLineNumber! > 0) newLines[change.newLineNumber! - 1] = tokens;
      } else if (change.type === 'delete') {
        if (change.lineNumber! > 0) oldLines[change.lineNumber! - 1] = tokens;
      } else if (change.type === 'insert') {
        if (change.lineNumber! > 0) newLines[change.lineNumber! - 1] = tokens;
      }
    }
  }
